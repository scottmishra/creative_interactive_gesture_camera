...
{ 
  ...
  //initialize the linearSolver
  linearSolver = new g2o::LinearSolverCholmod<g2o::BlockSolverX::PoseMatrixType>();
  //initialize the block solver object
  solver_ptr = new g2o::BlockSolverX(linearSolver);
  // create the algorithm to carry out the optimization, Levenberg Marquardt algorithm
  optimizationAlgorithm = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
...
void GraphOptimizer_G2O::optimizeGraph()
{
  ...
  optimizationAlgorithm->setUserLambdaInit(0.01);
...
int GraphOptimizer_G2O::getVertexIdx()
{
  return vertexIdx;
}
