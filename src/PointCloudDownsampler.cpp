			    ...
			    pcl::PointCloud<pcl::PointXYZRGB>::Ptr& downsampledPointCloudPtr)
{
    ...
    downsampledPointCloudPtr.reset(new pcl::PointCloud<pcl::PointXYZRGB> );
            ...
            for(int r2=r;r2<r+downsamplingStep;r2++)
            {
                for(int c2=c;c2<c+downsamplingStep;c2++)
                {
                    ...
                    if(pcl_isfinite (pointCloudPtr->points[r2*width+c2].x) &&
                       ...
                       0.3<pointCloudPtr->points[r2*width+c2].z &&
                       pointCloudPtr->points[r2*width+c2].z<5)
                    {
...
void PointCloudDownsampler::downsamplePointCloudColor( pcl::PointCloud<pcl::PointXYZRGB>::Ptr& pointCloudPtr,
			                                                 pcl::PointCloud<pcl::PointXYZRGB>::Ptr& downsampledPointCloudPtr)
{
    static int j;j=0;
    std::vector<double> xV;
