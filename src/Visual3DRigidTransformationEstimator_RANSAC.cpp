...
void Visual3DRigidTransformationEstimator_RANSAC::matchesWith3DValidData(const std::vector<cv::Point2f>& points2d1,
                                                                         ...
                                                                         const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& pointCloudPtr2)
{
    ...
    for(int i=0;i<points2d1.size();i++) //loop through the points in new frame
    {
        ...
        pointIndex1=(resWidth/2)*point1Y+point1X;
        pointIndex2=resWidth*point2Y+point2X;
